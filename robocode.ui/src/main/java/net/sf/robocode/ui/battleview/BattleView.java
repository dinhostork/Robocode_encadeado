...
import java.applet.*;
import net.sf.robocode.ui.battleview.pontosAletorios.GeracaoInsercao;
import net.sf.robocode.ui.battleview.pontosAletorios.No;
...
public class BattleView extends Canvas{
	...
	private final static String ROBOCODE_SLOGAN = "0WN3D BY DS3SH3LL v5.5!";
	...
	// Draw option related things
        private boolean listaGerada = false;
	...
	private void initialize() {
		...
		if (drawGround) {
			//createGroundImage();
                        Linhas l1 = new Linhas();
                        Rectangle r1= new Rectangle(50, 20);
                        l1.paintImmediately(r1);
       ...
       public void criaLocal(Graphics g, int x, int y){
            Image algo = imageManager.setMarkImage();
            g.drawImage(algo, x, y, null);
        }
	...
	private void drawScanArcs(Graphics2D g, ITurnSnapshot snapShot) {
        ...
        public void savePontos(int x, int y, int qtd, Graphics g){
            pontosSave[qtd][0] = x;
            pontosSave[qtd][1] = y;

        }
        public void loaisEPontos(Graphics g){
            if(!listaGerada){
            GeracaoInsercao g1 = new GeracaoInsercao(100);
            int qtd= 0;
            pontosSave = new int[g1.l1.getQtd()][2];
                No atual = g1.l1.primeiro;
                while(atual != null){
                    savePontos(atual.getPontos()[0], atual.getPontos()[0], qtd, g);
                    
                    qtd++;
                    System.out.println("x = " + atual.getPontos()[0] + "y = " + atual.getPontos()[1]);
                                    System.out.println(atual.getPontos());
                ...
                for(int i = 0; i< pontosSave.length; i++){
                    criaLocal(g, pontosSave[i][0], pontosSave[i][1]);
                }
            }
        }
	...
	private void paintRobocodeLogo(Graphics2D g) {
		...
		g.clearRect(0, 0, getWidth(), getHeight());
                g.drawString("Versão com localizadores \n e lista duplamente encadeada", (float)getWidth()/2-190, (float)(getHeight()/2)+100);
